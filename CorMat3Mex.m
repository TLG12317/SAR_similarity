function [X,y,info] = CorMat3Mex(G,b,I,J,OPTIONS,y)
...
%%% Last modified by Yan Gao and Defeng Sun on  November 23, 2009   
...
% reset pars
G       = G - tau*speye(n);   % reset G
G       = 0.5*(G+G');         % make G symmetric
Ind     = find(I==J);         % reset the diagonal part of b
b(Ind)  = b(Ind) - tau;
b0      = b;
% initial point
if ( nargin<6 )
...
while ( norm_b>tol && k1<maxit )
    ...
    k_inner = 0;
    while( k_inner <= maxitsub && f > f0 + sigma*0.5^k_inner*slope + 1.0e-6 )
...
if disp
    ...
    fprintf(' Number of func. evals(mexeig)   = %2.0f \n',f_eval)
